    ...
    public void addInterceptors(InterceptorRegistry registry) {
       /* //注册路由拦截器，自定义验证规则
        registry.addInterceptor(new SaRouteInterceptor((request, response, handler) -> {
            ...
            SaRouterUtil.match(Arrays.asList("/**"),Arrays.asList("/admin/login"), ()-> StpUtil.checkLogin());
            SaRouterUtil.match(Arrays.asList("/**"),Arrays.asList("/user/login"), ()-> StpUserUtil.checkLogin());
